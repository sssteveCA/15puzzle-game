...
if(isset($_SESSION['giocatore'],$_SESSION['logged']) && $_SESSION['giocatore'] != '' && $_SESSION['logged']){
    ...
    //Carica partita
    else if(isset($_POST['carica'],$_POST['ids']) && $_POST['carica'] == '1'){
        $giocatore = unserialize($_SESSION['giocatore']);
        $dati = array();
        $dati['id'] = $_POST['ids'];
        //unset($_SESSION['giocatore']);
        try{
            $salvataggio = new Salvataggio($dati);
            $errN = $salvataggio->getErrno();
            if($errN == 0){
                //dati di salvataggio
                $sequenza = $salvataggio->getSequenza();
                $spostamenti = $salvataggio->getSpostamenti();
                $tempo = $salvataggio->getTempo();
                if($spostamenti != null && $tempo != null){
                    //variabili di sessione da passare alla pagina del gioco(gioco.php)
                    $_SESSION['spostamenti'] = $salvataggio->getSpostamenti();
